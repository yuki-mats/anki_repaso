...
import 'utils/tracking_permission.dart';
import 'utils/paywall_manager.dart';
...
Future<bool> _shouldInitAppOpenAd(String uid) async {
  try {
    ...
    if (info.entitlements.active['Pro']?.isActive ?? false) return false;
...
Future<void> main() async {
  ...
  if (!kIsWeb) {
    ...
    final rcKey = defaultTargetPlatform == TargetPlatform.iOS ? iosKey : androidKey;
    ...
    if (await _shouldInitAppOpenAd(uid)) {
...
class _MainPageState extends State<MainPage> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    bottomNavigationBar: Column(
      ...
      children: [
        if (!kIsWeb && !_isPro) const BannerAdWidget(),
        BottomNavigationBar(
