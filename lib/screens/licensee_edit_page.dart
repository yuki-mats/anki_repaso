...
class LicenseeEditPage extends StatefulWidget {
  /// すでに選択済みの資格名を渡す
  final List<String> initialSelected;
  const LicenseeEditPage({
    Key? key,
    this.initialSelected = const [],
  }) : super(key: key);

  @override
  State<LicenseeEditPage> createState() => _LicenseeEditPageState();
}

class _LicenseeEditPageState extends State<LicenseeEditPage> {
  ...
  Future<void> _saveAndClose() async {
    ...
    try {
      await FirebaseFirestore.instance
          .collection('users')
          .doc(user.uid)
          .update({'selectedLicenseNames': _selected.toList()});
      Navigator.pop(context, _selected.toList());
    } catch (e) {
      debugPrint('Failed to update license names: $e');
      if (mounted) {
        ...
        child: Scaffold(
            ...
            children: [
              Card(
                ...
                child: const ListTile(
                  ...
                  subtitle: Text(
                      'その他の資格についても、順次、過去問を整備予定です！ぜひご登録ください。'),
                ),
              ),
              Padding(
          ...
          floatingActionButton: SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: Text(
                _selected.isEmpty ? '自分で問題を作成する' : '保存',
